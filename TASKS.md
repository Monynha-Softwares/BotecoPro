...
- [x] Criar projeto no Supabase, obtendo `SUPABASE_URL` e `SUPABASE_ANON_KEY`.
- [x] Modelar tabelas e funções no diretório `BotecoPro-Backend/database/supabase` replicando as entidades: `fornecedores`, `categorias`, `produtos`, `produtos_venda`, `mesas`, `pedidos`, `pedido_itens`, `vendas` etc.
- [ ] Habilitar o Realtime e configurar políticas de Row Level Security (RLS) para cada tabela.
- [x] Versionar scripts de seed e roles dentro de `database/supabase`.
...
- [x] Revisar `scripts/bootstrap.sh` e `codex_environment.sh` para garantir que não clonam repositórios quando já existem.
